...
- **Telegram bot**: an inverter summary that keeps itself up to date (edited in place every 15 seconds), with the time
  left on battery during a power cut; alerts for grid off / back, grid power above 5 kW, low battery, high load and a lost inverter connection (each as a summary with sound); several chat
